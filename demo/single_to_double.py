"""Generate a single-person motion followed by a two-person interaction."""
...
def main():
    ...
    second_prompt = str(
        cli_cfg.get("second_prompt", "one pivots and lifts their right leg to strike")
    )
    first_length = int(cli_cfg.get("first_length", 20))
    second_length = int(cli_cfg.get("second_length", 42))
    ...
    output_dir = Path(cli_cfg.get("output_dir", Path(cfg.demo_dir) / "single_to_double"))
    ...
    timeline = np.ones((2, len(joints1)), dtype=bool)
    timeline[1, :transition_frame] = False

    np.save(output_dir / "motion.npy", np.stack([motion1, motion2]))
    np.save(output_dir / "joints.npy", np.stack([joints1, joints2], axis=1))
    np.save(output_dir / "timeline.npy", timeline)
    (output_dir / "metadata.json").write_text(
        json.dumps(
            {
                "mode": "single_to_double",

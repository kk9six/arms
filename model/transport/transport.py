...
class Transport:
    ...
    def sample(self, x1, symmetric_t=False):
        ...
        t = th.rand((x1.shape[0], x1.shape[1])) * (t1 - t0) + t0
        if self.num_frame_per_block > 0:
            if symmetric_t:
                t = t.reshape(t.shape[0], 2, -1, self.num_frame_per_block)
                t[:, :, :, 1:] = t[:, :, :, 0:1]
                t[:, 1] = t[:, 0]
    ...
    def training_losses(
        ...
        t, x0, x1 = self.sample(x1, symmetric_t=symmetric_t)
        ...
        if self.model_type == ModelType.VELOCITY:
            terms["loss"] = mean_flat(((model_output - ut) ** 2), dim=dim)
            # t_expand = path.expand_t_like_x(t, xt)
            # terms["pred_x1"] = xt + (1 - t_expand) * model_output
        else:
...
class Sampler:
    ...
    def __get_sde_diffusion_and_drift(
        ...
        *,
        ...
        sde_drift = lambda x, t, model, **kwargs: self.drift(x, t, model, **kwargs) + diffusion_fn(
            x, t
